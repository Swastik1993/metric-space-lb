...
int main(int argc, char **argv) {
  ...
  cout << "rand test: " << r0 << " " << r1 << " " << r2 << "\n";
  srand(init); // reset so rest of run is identical
  ...
  auto start_lb_time = chrono::high_resolution_clock::now();

  TriSearch tri = TriSearch(adj_lst, nodes);
  size_t tri_mem = get_peak_rss_kb();
  auto stop_tri_end = chrono::high_resolution_clock::now();
  ...
  auto stop_sampling_end = chrono::high_resolution_clock::now();

  size_t exact_mem = 0;
  // cout << "Exact edges" << endl;
  if (EXACT) { 
    ELM_SPTree *elm_exact = new ELM_SPTree(adj_lst, nodes, k, sampling_size);
    exact_mem = get_peak_rss_kb();
    elm_exact->get_exact_landmarks();
  }
  auto stop_exact_end = chrono::high_resolution_clock::now();
  ...
  auto stop_h1_end = chrono::high_resolution_clock::now();
  ...
  auto stop_h2_end = chrono::high_resolution_clock::now();

  cout << "TriSearch Time : "
       << chrono::duration_cast<chrono::microseconds>(stop_tri_end - start_lb_time).count() << endl;
  cout << "ELM Sampling Time : "
       << chrono::duration_cast<chrono::microseconds>(stop_sampling_end - stop_tri_end).count() << endl;
  cout << "ELM Exact Time : "
       << chrono::duration_cast<chrono::microseconds>(stop_exact_end - stop_sampling_end).count() << endl;
  cout << "ELM H1 Time : "
       << chrono::duration_cast<chrono::microseconds>(stop_h1_end - stop_exact_end).count() << endl;
  cout << "ELM H2 Time : "
       << chrono::duration_cast<chrono::microseconds>(stop_h2_end - stop_h1_end).count() << endl;
  ...
  if (knn_queries > 0) {
    ...
    run_knn_metrics(
        ...
        elm_heuristic2,
        nodes,
        knn_k,                 // use your existing k as Top-K
        knn_queries,
        knn_pair_samples,
        init);
  }
  ...
  auto duration_exact_rt = chrono::duration_cast<chrono::microseconds>(dummy_s);
  auto duration_elm_rt = chrono::duration_cast<chrono::microseconds>(dummy_s);
  auto duration_h1_rt = chrono::duration_cast<chrono::microseconds>(dummy_s);
  auto duration_h2_rt = chrono::duration_cast<chrono::microseconds>(dummy_s);
      ...
      for (unsigned int j = i + 1; j < nodes; ++j) {
        if (adj_mat->at(i)->at(j) < -0.1) {
          if (lb->at(i)->at(j) > 0) {
            ...
            auto start_exact_rt = chrono::high_resolution_clock::now();
            dummy = elm_exact->lookup(i, j);
            auto stop_exact_rt = chrono::high_resolution_clock::now();
            duration_exact_rt += chrono::duration_cast<chrono::microseconds>(stop_exact_rt - start_exact_rt);
            ...
            auto start_h1_rt = chrono::high_resolution_clock::now();
            dummy = elm_heuristic1->lookup(i, j);
            auto stop_h1_rt = chrono::high_resolution_clock::now();
            duration_h1_rt += chrono::duration_cast<chrono::microseconds>(stop_h1_rt - start_h1_rt);

            auto start_h2_rt = chrono::high_resolution_clock::now();
            dummy = elm_heuristic2->lookup(i, j);
            auto stop_h2_rt = chrono::high_resolution_clock::now();
            duration_h2_rt += chrono::duration_cast<chrono::microseconds>(stop_h2_rt - start_h2_rt);
  ...
  cout << "Query time exact : " << duration_exact_rt.count() << endl;
  cout << "Query time sampling : " << duration_elm_rt.count() << endl;
  cout << "Query time heuristic1: " << duration_h1_rt.count() << endl;
  cout << "Query time heuristic2: " << duration_h2_rt.count() << endl;
